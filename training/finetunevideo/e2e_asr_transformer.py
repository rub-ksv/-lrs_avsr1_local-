...
from espnet.finetunevideo.decoder import Decoder
from espnet.finetunevideo.encoder import Encoder
from espnet.finetunevideo.ctcencoder import Encoder as ctcEncoder
from espnet.nets.pytorch_backend.transformer.initializer import initialize
from espnet.nets.pytorch_backend.transformer.label_smoothing_loss import (
    LabelSmoothingLoss,
)
from espnet.nets.pytorch_backend.transformer.mask import subsequent_mask
from espnet.finetunevideo.plot import PlotAttentionReport
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        if self.mtlalpha == 0.0:
            ...
            hs_pad, hs_mask = self.ctcencoder(hs_pad, hs_mask)
    ...
    def ctcencode(self, avhs_pad):
        self.eval()
        avhs_pad = torch.as_tensor(avhs_pad).unsqueeze(0)
        avhs_output, _ = self.ctcencoder(avhs_pad, None)
    ...
    def recognize(self, feat, recog_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        ctc_output = self.ctcencode(enc_output)
        if recog_args.ctc_weight > 0.0:
            lpz = self.ctc.log_softmax(ctc_output)
            lpz = lpz.squeeze(0)
        else:
            lpz = None
    ...
    def calculate_all_attentions(self, xs_pad, ilens, ys_pad):
        ...
        :param torch.Tensor ys_pad: batch of padded character id sequence
                                    tensor (B, Lmax)
